...
```{r,echo=FALSE}
read_count_distribution_name <- paste(sample_prefix,"aligned.tagged.cleaned.bam.readcount.txt",sep=".")
read_count_distribution <- read.table(paste("./qc_stats/",read_count_distribution_name,sep=""),
                             sep="\t",
                             header=F,
                             stringsAsFactors=F)

## Filter outs STAMPS that contribute too much reads based on the overall distribution
outliers <- subset(read_count_distribution,read_count_distribution$V1 > mean(read_count_distribution$V1) + 6*sd(read_count_distribution$V1))

read_count_distribution<- subset(read_count_distribution,read_count_distribution$V1 < mean(read_count_distribution$V1) + 5*sd(read_count_distribution$V1))
